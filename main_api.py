...
async def telegram_verify_code(req: TelegramVerifyCodeReq, user_id: int = Depends(get_current_user)):
    ...
    try:
        ...
        msg3_text = (
            ...
            "• `.يلا` أو `.ابدأ` : لتشغيل التبادل التلقائي الشامل بين جميع قنوات الحساب.\n"
            "  ← *مثال:* `.يلا 0 15 10` (ابدأ فوراً، موجة كل 15 دقيقة، مدة بقاء الإعلان 10 دقائق ثم حذفه).\n"
            "• `.تبادل_حملات` أو `.تبادل حملات` : لتشغيل التبادل التلقائي حصرياً داخل مجلد \"حملات\" فقط.\n"
            "  ← *مثال:* `.تبادل_حملات 0 15 10` (ابدأ فوراً داخل مجلد حملات، موجة كل 15 دقيقة، مدة بقاء الإعلان 10 دقائق).\n"
            ...
            "• `.حملات` أو `.فولدر` : تشغيل حملات مجمعة بالتناوب لكافة قنوات مجلد \"حملات\".\n"
